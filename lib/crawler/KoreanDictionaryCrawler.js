            ...
            if (elements) {
              for (let j = 0, len2 = elements.length; j < len2; j += 1) {
                ...
                if (text && /^[\sㄱ-ㅎ가-힣ㆍ^-]+$/.test(text)) {
                  ...
                  if (pos) {
                    word.pos = pos.map(value => value.replace(/「<font face="새굴림" style="font-size:13px">|<\/font>」/g, '').trim()) || null;
                  } else {
                    word.pos = null;
                  }
                  if (typeNCategory) {
                    const typeNCategoryTexts = typeNCategory.map(value => value.replace(/『<font face="새굴림" style="font-size:13px">|<\/font>』/g, '').trim()) || null;
                    if (typeNCategoryTexts) {
                      word.type = typeNCategoryTexts.filter(value => ['방언', '옛말', '북한어'].indexOf(value) >= 0) || null;
                      word.category = typeNCategoryTexts.filter(value => ['방언', '옛말', '북한어'].indexOf(value) < 0) || null;
                    } else {
                      word.type = null;
                      word.category = null;
                    }
